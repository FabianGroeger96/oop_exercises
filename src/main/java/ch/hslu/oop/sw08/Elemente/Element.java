...
import ch.hslu.oop.sw07.Elements.Temperatur;
...
public abstract class Element implements Comparable<Element> {
     ...
     */
    private Aggregatszustaende getAggregatsZu() {
        if (this.temp.getTempCelsius() <= this.schmelzpunkt) {
            return Aggregatszustaende.SOLID;
        } else if (this.temp.getTempCelsius() >= this.siedepunkt) {
            return Aggregatszustaende.GAS;
        } else {
            return Aggregatszustaende.LIQUID;
        }
    }
